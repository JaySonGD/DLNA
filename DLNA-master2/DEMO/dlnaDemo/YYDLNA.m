...
@interface YYDLNA()<CLUPnPServerDelegate, CLUPnPResponseDelegate>
...
@implementation YYDLNA

+ (YYDLNA *)sharedMRDLNAManager{
    static YYDLNA *instance = nil;
...
{
    NSLog(@"播放进度-trackDuration-%f--absTime-%f--relTime-%f",info.trackDuration,info.absTime,info.relTime);
}
...
- (void)upnpGetTransportInfoResponse:(CLUPnPTransportInfo *)info{
    NSLog(@"播放状态--currentTransportState:%@ ===currentTransportStatus:%@ ===currentSpeed:%@", info.currentTransportState, info.currentTransportStatus,info.currentSpeed);
